const CreateUIEditor = (ip_server) => {
	...
	var DEFPOS = [  {"top" : 925 , "left" : 0    , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 250  , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 520  , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 810  , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 1080 , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 1380 , "scale" : 0.12, "angle" : 0},
					{"top" : 925 , "left" : 1670 , "scale" : 0.12, "angle" : 0}];
	...
	var LastCFGUI = {}; // Configuración al dar click
	...
	function FillTemplate(UI, tmpl, SVG0 = 0){
		if(tmpl == undefined) {
			return "<div style='background-color: red;width: 1920px;height: 1080px;position: absolute;top: 0;"
					+ "	left: 0; font-size: 100px; font-weight: bold;align-items: center;display: flex; align-content: space-between;"
					+ "	justify-content: space-around;"
					+ "'> NOT FOUND TEMPLATE</div>";
		}
		...
		switch(data["info"]){
			...
			case "UIEditor":
				switch(data["action"]){
					case "0":
						let firstid = -1;
						for (const child of document.getElementById("p_uitype").children) {
							if(data["tmpls"][child.value] !== undefined){
								if(firstid < 0)  firstid = child.value;
								UITMPLS[child.value] = DecodeBase64(data["tmpls"][child.value]).replaceAll('"', "'");	
							} else {child.style.display = 'none';}
						}
						document.getElementById("p_uitype").value = firstid;
						CFGS = data["cfgs"];
						ChargeCFGs();
						onChangeCFG();
					break;
	...
	function ActUIType(){
		...
		switch(type){
			...
			case "1": // Doble 1
				document.getElementById("tmplscreen").srcdoc = FillTemplate(UIONFLY, UITMPLS[type]);
				document.getElementById('uidouble').style.top = SCREEN.offsetTop;
				document.getElementById('uidouble').style.left = '960px';
				document.getElementById('uidouble').style.width = '480px';
				document.getElementById('uidouble').style.height = '540px';
			...
			case "4": // Doble 3
				document.getElementById("tmplscreen").srcdoc = FillTemplate(UIONFLY, UITMPLS[type]);
				document.getElementById('uidouble').style.top = SCREEN.offsetTop  + 270;
				document.getElementById('uidouble').style.left = '480px';
				document.getElementById('uidouble').style.width = '960px';
				document.getElementById('uidouble').style.height = '270px';
				document.getElementById('uidouble').style.display = 'block';
			break;
			case "3": 
				document.getElementById("tmplscreen").srcdoc = FillTemplate(UIONFLY, UITMPLS[type]);
	...
	function mouseDown(e) {
		if(!PAINTING){
			...
			if(zona > 0){
				...
				LastCFGUI = JSON.parse(JSON.stringify(UIONFLY));
				PAINTING = true;
			}
		}else{
			FinishPaint();
	...
	function OnKeyDown(event){
		if(PAINTING){
			...
			switch(keyName){
				...
				break;
				case "Escape":
					UIONFLY = JSON.parse(JSON.stringify(LastCFGUI));
					FinishPaint();
				break;
				case "Enter":
					FinishPaint();
