...
const CreateReports = (ip_server) => {
	...
	{		
		...
		if (tDefects & 0x01) {
			s1 = { x: [MSGRep[14],MSGRep[15],MSGRep[16],MSGRep[17],MSGRep[18]], y: [0,0,0,0,0], name: MSGRep[20], type: 'bar', marker:{color: 'rgb(214,39,40)'},textposition: 'outside',hoverinfo: 'none'};
			for (i=1; i<7; i++) s1.y[i-1] = defects[i][1];
			s1.text = s1.y.map(String);
			series.push(s1);
		}
		if (tDefects & 0x02) {
			s2 = { x: [MSGRep[14],MSGRep[15],MSGRep[16],MSGRep[17],MSGRep[18]], y: [0,0,0,0,0], name: MSGRep[21], type: 'bar', marker:{color: 'rgb(31,119,180)'},textposition: 'outside',hoverinfo: 'none'};
			for (i=1; i<7; i++) s2.y[i-1] = defects[i][3];
			s2.text = s2.y.map(String);
			series.push(s2);						
		}
		if (tDefects & 0x04) {
			s3 = { x: [MSGRep[14],MSGRep[15],MSGRep[16],MSGRep[17],MSGRep[18]], y: [0,0,0,0,0], name: MSGRep[22], type: 'bar', marker:{color: 'rgb(44,160,44)'},textposition: 'outside',hoverinfo: 'none'};
		...
		layout.xaxis.title.text = MSGRep[28] + data.model + MSGRep[29] + ColorsByID_color[data.id_color] + "'";	
		s1 = { x: ['Z1','Z2','Z3','Z4','Z5'], y: [0,0,0,0,0], name: MSGRep[20], type: 'bar', marker:{color: 'rgb(225,107,0)'},textposition: 'outside',hoverinfo: 'none'};
		for (i=1; i<7; i++) {
