...
const CreateAdminUI = (ip_server) => {
	...
	var AutoPrint = false;
		...
		// Traducción con fallback: usa MSGAdm[key] o el innerHTML del label
		function trHTML(key, labelId){
			try{
				if(MSGAdm && typeof MSGAdm[key] !== 'undefined' && MSGAdm[key] !== null){ return MSGAdm[key]; }
				const el = document.getElementById(labelId || key);
	...
	function ProcessMSGAdmin(msg) {
		...
		switch(data["info"]){
			...
			case "ResultadoImprimir":
				...
				verificarModelo(data["infoCar"].modelo);
			break;
	...
	function FillTemplate(car, tmpl){
		...
		if(tmpl.includes("%%type%%.%%long%%")) {
			if(car.type && car.type.includes("4EVER")) {
				// Si es 4EVER, reemplazar todo el patrón solo con el tipo
				tmpl = tmpl.replace(/%%type%%\.%%long%%/g, car.type || "--");
				// Actualizar la lista de variables para excluir 'type' y 'long' ya procesadas
				vars = vars.filter(v => v !== "type" && v !== "long");
			}
		}
		...
		if(car.type && car.type.includes("4EVER")) {
			tmpl = tmpl.replace(/Modèle\s+4EVER\.L[12]/g, "Modèle 4EVER");
			tmpl = tmpl.replace(/4EVER\.L[12]/g, "4EVER");
		}
		return tmpl;
	}
	...
	function Inicilizar(){
		...
		switch(paramsAdm['Size']){
			...
			// Mensaje inicial de defectos (pantallas) con fallback
			if(document.getElementById("msgdefects")){
				document.getElementById("msgdefects").innerHTML = trHTML("lscreens","lscreens");
			}
		...
		for (let i=0; i<paises.length; i++) {
			...
			let nombrePais = paises[i].nombre;
			if (nombrePais === "ESPECIAL") {
				nombrePais = "JAPON";
			}
			elem.innerHTML = nombrePais;
			options_paises.appendChild(elem);
		}
	...
	function PideResultados() {
		...
		if(document.getElementById("TodosColores") && document.getElementById("TodosColores").checked) {
			colors_sql1 = "000";
    	}
		for (i=0; i<colores1.length; i++) {
	...
	function verificarModelo(modelo) {
		if (modelo.includes("4EVER") ) {
			document.getElementById("ImagenImpresion2").style.display = "block";
			document.getElementById("ImagenImpresion").style.display = "none";
			document.getElementById("ImagenImpresion3").style.display = "none";

		} else if(modelo.includes("APRL1") || modelo.includes("APRL2") || modelo.includes("APNL1") || modelo.includes("APNL2")){
			document.getElementById("ImagenImpresion3").style.display = "block";
			document.getElementById("ImagenImpresion").style.display = "none";
			document.getElementById("ImagenImpresion2").style.display = "none";
		
		} else {
			document.getElementById("ImagenImpresion").style.display = "block";
			document.getElementById("ImagenImpresion2").style.display = "none";
			document.getElementById("ImagenImpresion3").style.display = "none";
		}
	...
	function ChangeModeSizeDefects(elm){
		...
		if(document.getElementById("dscreens").checked){
			...
			if(document.getElementById("msgdefects")){
				document.getElementById("msgdefects").innerHTML = trHTML("ldrawdiscarded","ldrawdiscarded");
			}
		}
		else if(document.getElementById("drawndiscarded").checked){
			mode = FLAG_BLOBRAWDEFECTS;
			if(document.getElementById("msgdefects")){
				document.getElementById("msgdefects").innerHTML = trHTML("ldrawndiscarded","ldrawndiscarded");
			}
		}
		else if(document.getElementById("dactualfilter").checked){
			mode = FLAG_BLOBRAWDEFECTS | FLAG_FILTERRAWDEFECTS;
			if(document.getElementById("msgdefects")){
				document.getElementById("msgdefects").innerHTML = trHTML("ldactualfilter","ldactualfilter");
			}
		}
		ActualizeAdmin();
	}
